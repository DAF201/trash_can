...
class BiliBot:
    ...
    def reply(self, oid, type, root, parent, message, ater, plat=1):
        ...
        result = requests.post(
            url=reply_url, headers=sender_headers, data=reply_data)
        data_json = result.json()
        if data_json['code'] == 0:
    ...
    def get_medal(self, uid):
        ...
        if data_json['code'] == 0 and data_json['data']['list']:
            num = 10 if len(data_json['data']['list']) > 10 else len(
                data_json['data']['list'])
            for i in range(num):
    ...
    def send_msg(self, receiver_id, msg):
        ...
        result = requests.post(
            url=send_msg_url, headers=sender_headers, data=data)
        data_json = result.json()
        return data_json
        ...
        while True:
            if (num := self.checking_at()) > 0:
                print(time.strftime("%Y-%m-%d %H:%M:%S",
                      time.localtime()) + "发现被@{}次".format(num))
                if num > 10:
                    num = 10
                ...
                for i in range(num):
                    try:
                        ...
                        if root != 0:
                            ...
                            uid, name = s
                            if ater_id != uid:
                                message = self.get_common(
                                    uid, name) + '\n' + self.get_medal(uid)
                                print(message)
                                self.reply(oid, type, root, parent,
                                           nickname + message + time_str, ater_id)
                            else:
                                continue
                        time.sleep(5)
